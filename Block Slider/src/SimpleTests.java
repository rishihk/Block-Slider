...
public class SimpleTests
{
	...
	private static final Cell[][] testGrid1 =
	{
			...
			{ new Cell(WALL, 3, 0), new Cell(FLOOR, 3, 1), new Cell(FLOOR, 3, 2), new Cell(FLOOR, 3, 3), new Cell(FLOOR, 3, 4), new Cell(FLOOR, 3, 5),new Cell(WALL,2,6) },
			{ new Cell(WALL, 4, 0), new Cell(WALL, 4, 1), new Cell(WALL, 4, 2), new Cell(WALL, 4, 3), new Cell(WALL, 4, 4), new Cell(WALL, 4, 5),new Cell(WALL,4,6) } };

	private static ArrayList<Block> makeTest1Blocks()
	{
		ArrayList<Block> blocks = new ArrayList<Block>();
		//blocks.add(new Block(2, 2, 2, VERTICAL));
		//blocks.add(new Block(2, 1, 2, VERTICAL));
		blocks.add(new Block(1, 2,2, HORIZONTAL));
		return blocks;
	}
	...
	public static void main(String args[])
	{
		...
		Board newBoard = new Board(testGrid1, makeTest1Blocks());

		Block block = new Block(2, 1, 2, HORIZONTAL);
		System.out.println("Block is " + block);
		block.move(DOWN);
		System.out.println(block.getFirstRow());
		...
		System.out.println("Using testDescription1, first block is length " + blocks.get(0).getLength() + ", expected is 2.");
		System.out.println();
		...
		System.out.println("Location of grab is at (" + board.getGrabbedCell().getRow() + ", " + board.getGrabbedCell().getCol() + ") " + ", expected (2, 1).");
		System.out.println();

		board.moveGrabbedBlock(RIGHT);
		System.out.println("After moving block right one time game over is " + board.isGameOver() + ", expected is false.");
		System.out.println(board.toString());
		System.out.println();

		board.moveGrabbedBlock(RIGHT);
		System.out.println("After moving block right two times game over is " + board.isGameOver() + ", expected is true.");
		System.out.println(board.toString());
		System.out.println();
